...
export function YouTubeFeed() {
	...
	const rawGpName = session?.Meeting?.Name || "Barcelona";
	const gpName = rawGpName.replace(/grand prix/gi, "").trim();
		...
		<div className="w-full px-2 mt-6 relative group/feed">
			<div className="flex items-center justify-between mb-4 px-1 select-none">
				<h2 className="text-xs font-bold uppercase tracking-widest font-mono text-zinc-500 flex items-center gap-2">
					...
					• Resúmenes Oficiales: GP de {gpName}
				</h2>
			</div>

			<div className="relative w-full">
				
				{/* 👑 FLECHA IZQUIERDA: DISEÑO ORO 3D ESCULPIDO (Inspirado en image_fd1510.png) */}
				<button 
					onClick={() => scroll("left")}
				...
				>
					{/* SVG con geometría de chevron grueso y relieve metálico */}
					<svg 
					...
					>
						<defs>
							{/* Gradiente multi-parada para simular el relieve de lomo 3D chiseled */}
						...
						{/* Dibujamos un chevron ancho facetado en 3D */}
						<path 
				...
				{/* 👑 FLECHA DERECHA: DISEÑO ORO 3D ESCULPIDO (Inspirado en image_fd1510.png) */}
				<button 
				...
				>
					...
					>
						...
						{/* Dibujamos el chevron derecho grueso con terminación facetada */}
						<path 
